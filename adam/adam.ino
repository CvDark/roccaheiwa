...
// ✅ FIXED: Gunakan %20 untuk spasi + tambah /commercial
const String SERVER_URL = "https://overwealthy-lael-federally.ngrok-free.dev/final%20project%20rt/commercial";

const String DEVICE_ID  = "DEV001";

const String CHECK_STATUS_URL   = SERVER_URL + "/api/check_status.php?locker_id=" + String(LOCKER_ID);
const String NFC_SUBMIT_URL     = SERVER_URL + "/api/nfc_submit.php";
const String NFC_CHECK_MODE_URL = SERVER_URL + "/api/nfc_check_mode.php?device_id=" + DEVICE_ID;
...
void checkRegisterMode() {
    ...
    if (code == HTTP_CODE_OK) {
        ...
        if (wasRegister != registerMode) {
            Serial.println("[MODE] Switched to: " + String(registerMode ? "REGISTER" : "ACCESS"));
        }
    } else {
        // ⚠️ DEBUG: Tunjuk HTTP error code
        Serial.println("[MODE] HTTP Error: " + String(code) + " URL: " + NFC_CHECK_MODE_URL);
    }
    http.end();
}
...
void sendNFCScan(String uid, String purpose) {
    ...
    String payload = "{\"device_id\":\"" + DEVICE_ID + "\",\"uid\":\"" + uid + "\",\"purpose\":\"" + purpose + "\"}";
...
void checkServerStatus() {
    if (WiFi.status() != WL_CONNECTED || registerMode) return;
    HTTPClient http;
    ...
    if (code == HTTP_CODE_OK) {
        String res = http.getString();
        res.trim();
        if (res == "UNLOCK") unlockSolenoid();
        else if (res == "LOCK" && isUnlocked) lockSolenoid();
    } else {
...
void setup() {
    ...
    if (!versiondata) {
        ...
        while (1) { blinkLED(1, 100); delay(100); }
    }
...
void loop() {
    ...
    if (nfcReady && millis() - lastNFCScan >= NFC_COOLDOWN) {
        ...
        if (nfc.readPassiveTargetID(PN532_MIFARE_ISO14443A, uid, &uidLen, 500)) {
            ...
            Serial.println("[MODE] Current mode: " + String(registerMode ? "REGISTER" : "ACCESS"));
