...
try {
    // Buang scan lama yang pending dari device sama
    $pdo->prepare("
        DELETE FROM gm861_scan_queue 
        WHERE esp32_device_id = ? AND status = 'pending'
    ")->execute([$esp32_device_id]);

    // Simpan scan baru
    $pdo->prepare("
        INSERT INTO gm861_scan_queue 
        (esp32_device_id, scanned_data, status, created_at)
        VALUES (?, ?, 'pending', NOW())
