...
// Get NFC device_id — guna mana-mana locker yang ada device_id
// (user mungkin belum ada locker masa nak register card)
$nfc_device_id = '';
try {
    // Cuba dari locker user dulu
...
// Load all NFC cards for this user
$user_nfc_cards = [];
try {
    $stmt_cards = $pdo->prepare("SELECT id, nfc_uid, label, registered_at FROM user_nfc_cards WHERE user_id = ? ORDER BY registered_at ASC");
    $stmt_cards->execute([$user_id]);
    $user_nfc_cards = $stmt_cards->fetchAll(PDO::FETCH_ASSOC);
} catch (Exception $e) {
}
...
<head>
    ...
    <style>
        ...
        * {
        ...
        /* NFC */
        .nfc-registered-box {
            background: linear-gradient(135deg, #eff2ff, #f5f0ff);
            border: 1.5px solid #d4d9f8;
            border-radius: 12px;
            padding: 20px;
            display: flex;
            align-items: center;
        ...
        .nfc-center-icon {
            ...
            font-size: 28px;
            color: var(--primary);
            animation: nfcGlow 1.5s ease-in-out infinite;
        }
        ...
        <div class="container">
            <div class="d-flex align-items-center gap-4">
                ...
                <div>
                    ...
                    <div class="hero-meta">
                        <span><i
                                class="fas fa-envelope me-1"></i><?php echo htmlspecialchars($user['email'] ?? ''); ?></span>
    ...
    <div class="container py-4 pb-5" style="max-width:1100px;">
        ...
        <?php if ($message): ?>
            <div class="my-alert <?php echo $message['type'] === 'success' ? 'alert-ok' : 'alert-err'; ?>">
                <i
                    class="fas fa-<?php echo $message['type'] === 'success' ? 'check-circle' : 'exclamation-circle'; ?> fa-lg"></i>
                ...
                <div class="pcard">
                    ...
                    <div class="pcard-body">
                        ...
                        <div class="info-item">
                            ...
                            <span
                                style="background:#eff2ff;color:var(--primary);border-radius:20px;padding:3px 14px;font-size:.82rem;font-weight:700;">
                        ...
                        <div class="info-item">
                            ...
                            <span
                                class="info-value"><?php echo htmlspecialchars(!empty($user['institution']) ? $user['institution'] : '—'); ?></span>
                        </div>
                        <div class="info-item">
                            <span class="info-label">Phone</span>
                            <span
                                class="info-value"><?php echo htmlspecialchars(!empty($user['phone']) ? $user['phone'] : '—'); ?></span>
                        </div>
                        <div class="info-item">
                            <span class="info-label">Member Since</span>
                            <span
                                class="info-value"><?php echo isset($user['created_at']) ? date('d M Y', strtotime($user['created_at'])) : 'N/A'; ?></span>
                        </div>
                    </div>
                </div>
                        ...
                        <form method="POST">
                            ...
                            <div class="d-flex flex-column gap-3">
                                <div>
                                    ...
                                    <div class="pw-wrap">
                                        <input type="password" name="current_password" id="pw1" class="form-control"
                                            placeholder="Enter current password">
                                        <button type="button" class="pw-eye" onclick="togglePw('pw1',this)"><i
                                ...
                                <div>
                                    ...
                                    <div class="pw-wrap">
                                        <input type="password" name="new_password" id="pw2" class="form-control"
                                            placeholder="Min. 6 characters">
                                        <button type="button" class="pw-eye" onclick="togglePw('pw2',this)"><i
                                                class="fas fa-eye"></i></button>
                                    </div>
                                </div>
                                <div>
                                    ...
                                    <div class="pw-wrap">
                                        <input type="password" name="confirm_password" id="pw3" class="form-control"
                                            placeholder="Repeat new password">
                                        <button type="button" class="pw-eye" onclick="togglePw('pw3',this)"><i
                                                class="fas fa-eye"></i></button>
                                    </div>
                                </div>
        ...
        <div class="mt-4">
            <div class="pcard">
                ...
                <div class="pcard-body">
                    ...
                    <?php if (!empty($user_nfc_cards)): ?>
                        <div id="nfcCardList">
                            <?php foreach ($user_nfc_cards as $i => $card): ?>
                                <div class="nfc-registered-box" id="card-row-<?php echo $card['id']; ?>">
                                    <div class="nfc-card-icon">
                                        <i class="fas fa-id-card"></i>
                                    </div>
                                    <div style="flex:1;min-width:0;">
                                        <div style="font-size:.8rem;color:var(--muted);margin-bottom:2px;">
                                            <?php echo htmlspecialchars($card['label'] ?? 'Card ' . ($i + 1)); ?>
                                        </div>
                                        <div class="nfc-uid"><?php echo htmlspecialchars($card['nfc_uid']); ?></div>
                                        <div style="font-size:.78rem;color:var(--muted);margin-top:6px;">
                                            <i class="fas fa-clock me-1"></i>
                                            <?php echo date('d M Y, H:i', strtotime($card['registered_at'])); ?>
                                        </div>
                                    </div>
                                    <div style="display:flex;flex-direction:column;gap:8px;align-items:flex-end;">
                                        <span class="badge-active"><i class="fas fa-check-circle me-1"></i>Active</span>
                                        <?php if (!empty($nfc_device_id)): ?>
                                        <button class="btn-replace" style="font-size:.78rem;padding:6px 14px;"
                                            onclick="removeCard(<?php echo $card['id']; ?>, '<?php echo htmlspecialchars($card['nfc_uid']); ?>')"
                    ...
                    <?php else: ?>
                        <div class="nfc-empty-box" id="nfcEmptyBox">
                    ...
                    <?php if (!empty($nfc_device_id)): ?>
                        <div class="mt-3">
                            <button class="btn-nfc-reg" onclick="startNFCRegister()">
                                ...
                                <?php echo empty($user_nfc_cards) ? 'Register NFC Card' : 'Add Another Card'; ?>
                            </button>
                        </div>
                    <?php endif; ?>
                    ...
                    <!-- Register Panel (shared for adding any new card) -->
                    <div id="nfcPanel" style="display:none;" class="mt-4">
                        ...
                        <div id="nfcWaiting" class="text-center">
                            ...
                            <div id="nfcHint" style="font-size:.875rem;color:var(--muted);margin-bottom:20px;">Hold your card near the NFC reader at any of your lockers</div>
                            <div class="timer-wrap">
                                ...
                                <div class="timer-bg">
                                    <div class="timer-fill" id="nfcTimerBar"></div>
                                </div>
                            </div>
    ...
    <script>
        ...
        function startNFCRegister() {
            ...
            .then(d => {
                ...
                document.getElementById('nfcHint').textContent = 'ESP32 switching to register mode... Tap your NFC card at any locker reader in ~3 seconds';
        ...
        function removeCard(cardId, uid) {
            if (!confirm('Remove NFC card (' + uid + ') from your account?')) return;
            fetch('api/nfc_remove_card.php', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
            ...
            .then(d => {
                if (d.success) {
                    ...
                    const list = document.getElementById('nfcCardList');
                    if (list && list.querySelectorAll('.nfc-registered-box').length === 0) {
                        location.reload();
                    }
                } else {
                    alert('Failed to remove: ' + d.message);
                }
            })
            .catch(() => alert('Network error removing card.'));
        }
    </script>
</body>
